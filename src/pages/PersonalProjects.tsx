      ...
      <div className="grid grid-cols-2 gap-6 h-full">
        ...
        <div className="col-span-1 relative flex items-center justify-center">
          ...
          <div className="absolute bottom-0 right-4 flex gap-2">
            {hasPrev && <Button icon={BiSolidHandLeft} iconSize={32} onClick={() => setProjectIndex((i) => i - 1)} />}
            {hasNext && <Button icon={BiSolidHandRight} iconSize={32} onClick={() => setProjectIndex((i) => i + 1)} />}
          </div>
        </div>
      </div>
    </div>
